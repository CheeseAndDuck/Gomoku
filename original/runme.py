...
class Board(object):  # 棋盘类（原有逻辑不变
    ...
    def current_state(self):
        ...
        if len(self.states) % 2 == 0:
            square_state[3][:, :] = 1.0
        reversed_state = square_state[:, ::-1, :].copy()  # 这一步是为了消除负步长
        return reversed_state

    def do_move(self, move):  # 添加防御性检查 
...
class MetaZeta(threading.Thread):
    save_ParaFreq = 2  # 每save_ParaFreq局保存一次进度
    MAX_Games = 110  # 总训练局数
    ...
    def evaluate(self, num_games=10):  # 评估函数
