...
class Board(object): # "棋盘类
    ...
    def current_state(self):
        ...
        if len(self.states) % 2 == 0:
            square_state[3][:, :] = 1.0
        reversed_state = square_state[:, ::-1, :].copy()  # 消除负步长
...
class Game(): 
    ...
    def selfPlay(self, player, Index=0):
        ...
        while True:
            board_state = torch.tensor(self.board.current_state(), dtype=torch.float32).unsqueeze(0).to(self.device)  # 棋盘状态
...
class MCTSPlayer():  # AI棋手
    ...
    def getAction(self, board, flag_is_train,board_state):
        ...
        if len(emptySpacesBoard) > 0:
            acts, probs = self.MCTS.getMoveProbs(board, flag_is_train, board_state) # 获取落子位置和概率
...
class MetaZeta(threading.Thread):
    save_ParaFreq = 5  # 每save_ParaFreq局保存一次进度
    MAX_Games = 60    # 总训练局数
    ...
    def save_training_log(self, avg_loss, avg_kl):  # 保存训练记录
        try:
            ...
            with open(self.training_log_path, 'a', newline='', encoding='utf-8') as f:
                ...
                writer.writerow([
                    ...
                    avg_loss,
                    avg_kl,
                    self.MCTSPlayer.MCTS.alpha,  # 记录当前α值
                    self.LRfctor,
                    self.trainBatchSize,
    ...
    def save_training_checkpoint(self, current_game, optimizer, save_path="checkpoints"):  # 保存完整训练进度
        ...
        checkpoint = {
            ...
            "alpha_value": self.MCTSPlayer.MCTS.alpha,
    ...
    def load_training_checkpoint(self, checkpoint_path):  # 加载训练进度，恢复所有关键状态
        ...
        try:
            ...
            if "alpha_value" in checkpoint:  # 恢复α值
                self.MCTSPlayer.MCTS.alpha = checkpoint["alpha_value"]
    ...
    def train(self):
        ...
        optimizer = optim.Adam(self.policy_net.parameters(), lr=self.policy_net.learningRate * self.policy_net.LRfctor)  # 创建优化器
        ...
        for oneGame in range(start_game, self.MAX_Games):
            ...
            if self.flag_is_train:
                ...
                if current_game_num % self.save_ParaFreq == 0:
                    self.policy_net.save_model(f'D:/Graduate_student_without_testing/985/10th-version5/models2/{current_game_num}tactic.model')
                    
                    if hasattr(self.value_net, 'save_model'):
                        try:
                            self.value_net.save_model(f'D:/Graduate_student_without_testing/985/10th-version5/models2/{current_game_num}evaluateValue.model')
                        except Exception as e:
                ...
                if self.loaded_checkpoint is not None:
                    ...
                    try:
                        model_path = f'D:/Graduate_student_without_testing/985/10th-version5/models2/{current_game_num}tactic.model'
                        self.policy_net.load_model(model_path)
                        value_path = f'D:/Graduate_student_without_testing/985/10th-version5/models2/{current_game_num}evaluateValue.model'
        ...
        self.policy_net.save_model("D:/Graduate_student_without_testing/985/10th-version5/models2/final_tactic.model")
        if hasattr(self.value_net, 'save_model'):
            try:
                self.value_net.save_model("D:/Graduate_student_without_testing/985/10th-version5/models2/final_evaluateValue.model")
            except Exception as e:
    ...
    def update_alpha_based_on_performance(self, win_rate=None, value_loss=None):
        ...
        self.alpha = min(self.max_alpha, self.alpha + self.alpha_growth_rate * growth_factor)   # 更新α值
        self.alpha_history.append(self.alpha)  # 记录α值变化

    def battle_mode(self):
        try:
            self.policy_net.load_model("models2/Version1.model")
            if hasattr(self.value_net, 'load_model'):
                try:
                    self.value_net.load_model("models2/Version1_value.model")
                except Exception as e:
    ...
    def save_to_csv(self, play_data, game_index, winner, filepath='D:/Graduate_student_without_testing/985/10th-version5/play_chess.csv'):
        seq = list(play_data)
    ...
    def evaluate(self, num_games=10):  # 评估函数，同时更新α值
        original_show_flag = self.game.flag_is_shown
        self.game.flag_is_shown = True
        ...
        self.drawScrollText(f"当前胜率: {win_rate:.2%}, α值: {self.MCTSPlayer.MCTS.alpha:.3f}")
